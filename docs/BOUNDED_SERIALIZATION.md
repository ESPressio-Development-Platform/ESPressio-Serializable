...
Floating metadata requires IEEE binary32/binary64 and C++17 compiler support for constant-evaluated `__builtin_bit_cast` (validated on host GCC 13 and the ESP32 GCC 14 CI target). Fingerprints preserve signed zero and NaN payload wire bits. The edge suite denies heap allocation during floating conversion, including first use and malformed input. ESP32 CI instantiates all three bounded codecs with floating and fixed-container fields.
