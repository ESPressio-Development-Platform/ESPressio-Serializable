...
public:
    Security::SecurityResult Protect(
        ...
        Security::SecurityBuffer& output,
        const Security::DataProtectionContext& context = {}
    ) override {
        if (plaintext == nullptr && plaintextSize != 0) {
            return Security::SecurityResult::Fail(Security::SecurityError::InvalidArgument, "bad input");
        }
        output.clear();
        output.push_back(ContextByte(context));
        for (std::size_t i = 0; i < plaintextSize; ++i) {
            output.push_back(static_cast<uint8_t>(plaintext[i] ^ 0xA5u));
        }
    ...
    Security::SecurityResult Unprotect(
        ...
        Security::SecurityBuffer& plaintext,
        const Security::DataProtectionContext& context = {}
    ) override {
        plaintext.clear();
        ...
        for (std::size_t i = 1; i < inputSize; ++i) {
            plaintext.push_back(static_cast<uint8_t>(input[i] ^ 0xA5u));
        }
        return Security::SecurityResult::Ok(true);
    }
...
private:
    static uint8_t ContextByte(const Security::DataProtectionContext& context) {
        ...
        for (std::size_t i = 0; i < context.Size; ++i) {
            value = static_cast<uint8_t>((value * 33u) ^ context.Data[i]);
        }
        return value;
    }
};
...
int main() {
    ...
    Serializable::SerializationBuffer<uint8_t> protectedBytes;
    ...
    auto loaded = Serializable::DeserializeProtectedBinary(
        protectedBytes.data(), protectedBytes.size(), restored, protection
    );
    ...
    auto rejectedResult = Serializable::DeserializeProtectedBinary(
        protectedBytes.data(), protectedBytes.size(), rejected, wrongContext
    );
    ...
    Serializable::SerializationBuffer<uint8_t> ordinary;
    assert(Serializable::SerializeBinary(source, ordinary, nullptr).Success());
    TestConfiguration ordinaryRestored;
    assert(Serializable::DeserializeBinary(
        ordinary.data(), ordinary.size(), ordinaryRestored, nullptr
    ).Success());
    assert(ordinaryRestored.Name() == "recorder");

    protection.MaximumArchiveBytes = 1;
    Serializable::SerializationBuffer<uint8_t> limited;
    assert(Serializable::SerializeProtectedBinary(source, limited, protection).Status ==
        Serializable::ProtectedSerializationStatus::PayloadTooLarge);
