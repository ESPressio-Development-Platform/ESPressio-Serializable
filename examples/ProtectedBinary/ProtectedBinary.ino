 ...
 * Inherited Memory Total: sizeof(Serializable::Serializable<WiFiLikeConfiguration>) [0 bytes dynamic allocation]
 * Members:
 * - _password (std::string): 24 bytes [Capacity + 1 bytes when capacity exceeds 15-byte SSO]
 * - _channel (uint8_t): 1 bytes [0 bytes dynamic allocation]
 * Total Memory: sizeof(Serializable::Serializable<WiFiLikeConfiguration>) + 25 bytes known members [_password: Capacity + 1 bytes when capacity exceeds 15-byte SSO]
